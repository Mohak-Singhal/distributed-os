...
pub const QUIC_FRAME_HANDSHAKE_REQ: u8 = 10;
pub const QUIC_FRAME_HANDSHAKE_RESP: u8 = 11;
pub const QUIC_FRAME_STREAM_DATA: u8 = 12;
pub const QUIC_FRAME_ACK: u8 = 13;
pub const QUIC_FRAME_FIN: u8 = 14;
pub const QUIC_FRAME_FIN_ACK: u8 = 15;

pub const QUIC_CHUNK_SIZE: usize = 8192;
...
pub async fn send_file_quic(
    ...
    let num_chunks = ((total_size + QUIC_CHUNK_SIZE as u64 - 1) / QUIC_CHUNK_SIZE as u64) as usize;
    ...
    while handshake_attempts < 5 {
        let mut req = vec![QUIC_FRAME_HANDSHAKE_REQ];
        req.extend_from_slice(&total_size.to_be_bytes());
        req.push(num_streams as u8);
        ...
        tokio::select! {
            res = socket.recv(&mut buf) => {
                if let Ok(n) = res {
                    if n > 0 && buf[0] == QUIC_FRAME_HANDSHAKE_RESP {
    ...
    if !handshake_success {
        return Err(anyhow!("QUIC handshake failed — remote device did not respond"));
    }

    // 2. Data Transmission Phase (Multiplexed Streams)
    let mut next_seq = 0usize;
    let mut acked_seqs = HashSet::new();
    let mut sent_times = HashMap::new();
    let mut last_sent_time = HashMap::new();
    
    let mut cwnd = 12usize;
    let mut rtt_ms = 40.0;
    let mut rtt_var_ms = 8.0;
    let mut retransmits = 0u64;
    let mut total_sent = 0u64;

    let (ack_tx, mut ack_rx) = mpsc::channel::<usize>(2000);
    ...
    while acked_seqs.len() < num_chunks {
        ...
        while next_seq < num_chunks && (next_seq - acked_seqs.len()) < cwnd {
            let offset = next_seq * QUIC_CHUNK_SIZE;
            let end = (offset + QUIC_CHUNK_SIZE).min(file_data.len());
            let chunk = &file_data[offset..end];

            // Multiplex across stream IDs: next_seq % num_streams
            let stream_id = (next_seq % num_streams) as u8;

            let mut packet = vec![QUIC_FRAME_STREAM_DATA];
        ...
        tokio::select! {
                ...
                if let Some(seq) = maybe_seq {
                    ...
                    if acked_seqs.insert(seq) {
                        ...
                        if let Some(ref cb) = progress_cb {
                            cb((acked_seqs.len() * QUIC_CHUNK_SIZE).min(file_data.len()) as u64, total_size);
                        }
                    }
                }
            }
            _ = tokio::time::sleep(timeout_duration) => {
                ...
                for seq in acked_seqs.len()..next_seq {
                    if !acked_seqs.contains(&seq) {
                        ...
                        if should_retransmit {
                            let offset = seq * QUIC_CHUNK_SIZE;
                            let end = (offset + QUIC_CHUNK_SIZE).min(file_data.len());
                            let chunk = &file_data[offset..end];
                            let stream_id = (seq % num_streams) as u8;
                ...
                cwnd = (cwnd / 2).max(4); // Halve window
            }
        }
    }
    ...
    while fin_attempts < 5 {
        let fin_packet = vec![QUIC_FRAME_FIN];
...
pub async fn receive_file_quic(
    save_path: &Path,
    listen_port: u16,
    expected_size: u64,
    progress_cb: Option<Arc<dyn Fn(u64, u64) + Send + Sync>>,
    ...
    let mut buf = vec![0u8; QUIC_CHUNK_SIZE + 32];
    
    let mut file_buf = vec![0u8; expected_size as usize];
    let num_chunks = ((expected_size + QUIC_CHUNK_SIZE as u64 - 1) / QUIC_CHUNK_SIZE as u64) as usize;
    ...
    loop {
        ...
        match frame_type {
            QUIC_FRAME_HANDSHAKE_REQ => {
                let resp = vec![QUIC_FRAME_HANDSHAKE_RESP];
                socket.send_to(&resp, addr).await?;
            }
            QUIC_FRAME_STREAM_DATA => {
                if n < 12 { continue; }
                let _stream_id = buf[1];
                let mut seq_bytes = [0u8; 8];
                seq_bytes.copy_from_slice(&buf[2..10]);
                let seq = usize::from_be_bytes(seq_bytes);

                let mut len_bytes = [0u8; 2];
                len_bytes.copy_from_slice(&buf[10..12]);
                let len = u16::from_be_bytes(len_bytes) as usize;

                if n >= 12 + len {
                    let offset = seq * QUIC_CHUNK_SIZE;
                    if offset + len <= file_buf.len() {
                        file_buf[offset..offset+len].copy_from_slice(&buf[12..12+len]);
                        if received_chunks.insert(seq) {
                            if let Some(ref cb) = progress_cb {
                                cb(received_chunks.len() as u64 * QUIC_CHUNK_SIZE as u64, expected_size);
                            }
                        }
                    }
                    // Send ACK
                    let mut ack = vec![QUIC_FRAME_ACK];
                    ack.extend_from_slice(&seq.to_be_bytes());
                    socket.send_to(&ack, addr).await?;
                }
            }
            QUIC_FRAME_FIN => {
                let resp = vec![QUIC_FRAME_FIN_ACK];
                socket.send_to(&resp, addr).await?;
...
pub async fn receive_file_quic_with_socket(
    socket: UdpSocket,
    save_path: &Path,
    expected_size: u64,
...
mod tests {
    ...
    #[tokio::test]
    async fn test_quic_transport_end_to_end() -> Result<()> {
        ...
        let recv_handle = tokio::spawn(async move {
            ...
            receive_file_quic_with_socket(rx_socket, &dst_path_clone, 50 * 1024, Some(Box::new(cb))).await
        });

        let progress_cb = Arc::new(|_sent: u64, _total: u64| {});
        let metrics = send_file_quic(&src_path, rx_addr, 4, Some(progress_cb)).await?;
